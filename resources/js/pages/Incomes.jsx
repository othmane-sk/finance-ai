...
export default function Incomes({ currency }) {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const filtered = res.data.filter(c => c.type === 'income');
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white font-display">Incomes</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">Track and manage incoming cashflow logs.</p>
        </div>
        <button
          onClick={handleOpenCreateModal}
          className="flex items-center justify-center space-x-1.5 px-4 py-2.5 rounded-xl bg-lime-500 text-slate-950 text-xs font-bold shadow-lg shadow-lime-500/10 hover:bg-lime-450 transition-all cursor-pointer"
        >
      ...
      <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 shadow-sm">
        ...
        <input
          ...
          className="px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-850 text-slate-850 dark:text-white text-xs focus:outline-none"
        />
        <input
          type="date"
          value={endDate}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2.5 rounded-xl bg-white dark:bg-slate-955 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-250 text-xs focus:outline-none"
                >
                  {categories.map((c) => (
                    <option key={c.id} value={c.id}>{c.name}</option>
                  ))}
              ...
              <div className="flex space-x-3 pt-3">
                ...
                <button
                  type="submit"
                  className="flex-1 py-2.5 rounded-xl bg-lime-500 hover:bg-lime-450 text-slate-955 text-xs font-bold shadow-lg shadow-lime-500/10 transition-all cursor-pointer"
                >
